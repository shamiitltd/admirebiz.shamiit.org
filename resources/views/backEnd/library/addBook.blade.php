...
@section('mainContent')
    ...
    <section class="admin-visitor-area">
        <div class="container-fluid p-0">
            ...
            @if (isset($editData))
                {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'route' => ['update-book-data', $editData->id], 'method' => 'POST', 'enctype' => 'multipart/form-data']) }}
            @else
                            ...
                            <div class="row mb-30">
                                <div class="col-lg-3">
                                    <div class="primary_input ">
                                        <label class="primary_input_label" for="">@lang('library.book_title') <span
                                ...
                                <div class="col-lg-3">
                                    <div class="primary_input ">
                                        <label class="primary_input_label" for="">@lang('library.book_category') <span
                                                class="text-danger"> *</span> </label>
                                        <select
                                            ...
                                            @foreach ($categories as $key => $value)
                                                @if (isset($editData))
                                                    <option
                                                        value="{{ $value->id }}"
                                                        {{ $value->id == $editData->book_category_id ? 'selected' : '' }}>
                                                        {{ $value->category_name }}</option>
                                                @else
                                                    <option
                                                        value="{{ $value->id }}"
                                                        {{ old('book_category_id') != '' ? (old('book_category_id') == $value->id ? 'selected' : '') : '' }}>
                                                        {{ $value->category_name }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                            ...
                                            @foreach ($subjects as $key => $value)
                                                @if (isset($editData))
                                                    <option value="{{ $value->id }}"
                                                        {{ $value->id == $editData->book_subject_id ? 'selected' : '' }}>
                                                        {{ $value->subject_name }}</option>
                                                @else
                                                    <option value="{{ $value->id }}"
                                                        {{ old('subject') != '' ? (old('subject') == $value->id ? 'selected' : '') : '' }}>
                                                        {{ $value->subject_name }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                            ...
                            <div class="row md-20">
                                <div class="col-lg-12">
                                    <div class="primary_input">
                                        ...
                                        <textarea class="primary_input_field form-control generated-text" cols="0" rows="4" name="details"
